...
class DualEncoderEpsNetwork(nn.Module):
    ...
    def __init__(self, config, lr_init):
        super().__init__()
        self.config = config
        ...
        lr =  lr_init #initialization consistent with lr from other config files (so I don't have to add another input, which would change a ton)
        self.param_names = ["edge_encoder_global", "edge_encoder_local", 
                  "encoder_global", "encoder_local", 
                  "grad_global_dist_mlp", "grad_local_dist_mlp"]
        self.inner_lrs = {k: torch.tensor(lr,requires_grad=True) for k in self.param_names}
