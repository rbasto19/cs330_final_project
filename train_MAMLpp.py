...
if __name__ == '__main__':
    ...
    if args.curriculum == True:
        ...
        if args.curriculum_type == 0:
            ...
            # define lists of indices in steps of 5, that correspond to section of 
            # dataset, then in train just sample from that depending on iteration
            starting_pct = 0.1 #start with 10% of data
            idx_lists = []
            step_length = epoch_size/epoch_split
            inc = round((1/starting_pct)**(1/(epoch_split-1)),3)+0.001
            for i in range(epoch_split):
                idx_lists.append(np.arange(0,len(train_set)*min(starting_pct*inc**(i),1), num_examples_per_task))
    ...
    def train(it):
        ...
        if MAML == True:
            ...
            if args.curriculum_type == 0 or args.curriculum_type == 1:
                ...
                section = min(int((it-1)/(step_length*size_train_MAML)), epoch_split-1) #just to be safe in edge case that dataset weird size, then just sample a bit more from all dataset
                print(section)
        ...
        #previous approach
            ...
            if args.curriculum_type == 0 or args.curriculum_type == 1:
                for _ in range(batch_size_outer_loop):
                    batch = batch + next(train_iterator).to(args.device).to_data_list()
            elif args.curriculum_type == 2:
                section = min(int((it_geodiff-1)/(step_length*size_train_geodiff)), epoch_split-1)
                print(section)
                idxs = np.random.choice(idx_lists[section], args.outer_batch_size, replace=False)
                for i in idxs:
                    for j in range(num_examples_per_task):
                        batch.append(train_set[int(i)+j].to(args.device)) #okay to not split since here we don't do inner loop steps
